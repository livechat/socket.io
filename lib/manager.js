...
Manager.prototype.handleRequest = function (req, res) {
  ...
  if (data.static || !data.transport && !data.protocol) {
    if (data.static && this.enabled('browser client')) {
      ...
      res.send(200, 'Welcome to socket.io.');
  ...
  if (data.protocol != protocol) {
    res.send(500, 'Protocol version not supported.');
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  function error (err) {
    ...
    res.send('io.j[' + data.query.jsonp + '](new Error("' + message + '"));');
  };
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      if (handshakeData.setCookie) {
        if (!(handshakeData.setCookie instanceof Array)){
          handshakeData.setCookie = [handshakeData.setCookie]
        }

        for (var cookie in handshakeData.setCookie){
          res.setHeader("Set-cookie", handshakeData.setCookie[cookie]);
        }
        delete handshakeData.setCookie;
      }
      ...
      writeErr(403, newData || 'handshake unauthorized');
...
Manager.prototype.authorize = function (data, fn) {
  if (this.get('authorization')) {
    ...
    this.get('authorization').call(this, data, function (err, authorized, status) {
      self.log.debug('client ' + authorized ? 'authorized' : 'unauthorized');
      fn(err, authorized, status);
    });
  } else {
